...
\noindent The node supports two navigation modes: CAD mode, which uses pre-built maps with internal path planning algorithms, and SLAM mode, which uses the ROS navigation stack (map\_server, AMCL, move\_base) for real-time localization and obstacle avoidance. In SLAM mode, the node automatically launches the required navigation stack components as a subprocess, providing a self-contained navigation solution.

\noindent The node provides both a service-based interface (blocking) and an action-based interface (non-blocking with real-time feedback and goal preemption support). Both interfaces are available simultaneously, allowing client nodes to choose the appropriate communication pattern. When social distance mode is enabled, the passing distance value is read from the \texttt{cultureKnowledgeBaseInput.dat} file in the \texttt{behaviorController} package.
...
\begin{figure}[H]
	...
	\dirtree{%
		...
		.5 navigation.launch.
		.4 maps.
		.5 environmentMap.pgm.
		.5 environmentMap.yaml.
		.4 param.
		.5 move\_base\_params.yaml.
		.5 costmap\_common\_params.yaml.
		.5 local\_costmap\_params.yaml.
		.5 global\_costmap\_params.yaml.
		.5 base\_local\_planner\_params.yaml.
...
\verb|navigationMode| & \verb|CAD, SLAM| & Specifies the navigation mode. CAD uses internal path planning; SLAM launches the ROS navigation stack automatically. \\ \hline
\end{tabularx}
\end{table}
...
\verb|/robotNavigation/set_goal| & \(<x> <y> <\theta>\) & Define navigation goal pose. Returns ``1'' on success, ``0'' on failure. \\ \hline
\verb|/robotNavigation/set_pose| & \(<x> <y> <\theta>\) & Set the robot pose. In SLAM mode, publishes to the AMCL initialpose topic. \\ \hline
\end{tabularx}
\end{table}
...
In addition to the service interface, the node provides an action-based interface for non-blocking navigation with real-time feedback and goal preemption support. Both action servers are available simultaneously alongside the service servers. The actions supported are summarized in Table~\ref{tab:actions supported}.

\begin{table}[H]
\centering
\caption{Actions Provided by the robotNavigation node.}
\label{tab:actions supported}
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\textbf{Action} & \textbf{Type} & \textbf{Description} \\ \hline
\verb|/robotNavigation/set_goal| & \verb|cssr_system/setGoalAction| & Navigate to a goal pose with feedback and preemption. \\ \hline
\verb|/robotNavigation/set_pose| & \verb|cssr_system/setPoseAction| & Set the robot pose with status feedback. \\ \hline
\end{tabularx}
\end{table}

\noindent The \verb|setGoal| action accepts a goal pose ($x$, $y$, $\theta$) and returns the navigation result along with the final robot pose. During execution, the action server publishes periodic feedback containing the distance remaining to the goal and the current robot pose. The action can be preempted (cancelled) by the client at any time.

\noindent The \verb|setPose| action accepts a pose ($x$, $y$, $\theta$) and sets the robot's position. In SLAM mode, the pose is forwarded to the AMCL initialpose topic. The action provides status feedback during execution.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/navigationFolder.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/rosrun_navigation_CAD.png}
...
\subsubsection*{Sending a Goal via Action}
The action interface provides non-blocking navigation with real-time feedback. To send a goal via the action interface:
\begin{lstlisting}[style=withoutNumbering, language=bash]
rostopic pub -1 /robotNavigation/set_goal/goal cssr_system/setGoalActionGoal
  "goal: {goal_x: 2.0, goal_y: 6.0, goal_theta: 270.0}"
\end{lstlisting}
To monitor feedback during navigation:
\begin{lstlisting}[style=withoutNumbering, language=bash]
rostopic echo /robotNavigation/set_goal/feedback
\end{lstlisting}
To cancel an active navigation goal:
\begin{lstlisting}[style=withoutNumbering, language=bash]
rostopic pub -1 /robotNavigation/set_goal/cancel actionlib_msgs/GoalID "{}"
\end{lstlisting}

\subsubsection*{Setting Robot Pose}
To set the robot's pose via the service interface while \verb|robotNavigation| node is running in SLAM mode:
\begin{lstlisting}[style=withoutNumbering, language=bash]
rosservice call /robotNavigation/set_pose 2.0 7.8 270.0
\end{lstlisting}
To set the robot's pose via the action interface:
\begin{lstlisting}[style=withoutNumbering, language=bash]
rostopic pub -1 /robotNavigation/set_pose/goal cssr_system/setPoseActionGoal
  "goal: {pose_x: 2.0, pose_y: 7.8, pose_theta: 270.0}"
\end{lstlisting}
...
\noindent The \texttt{robotNavigationTest} folder is structured to support unit testing of the \verb|robotNavigation| unit test Node. The \texttt{src} directory contains \texttt{robotNavigationDriver.cpp}, which serves as the driver for test execution, \texttt{robotNavigationTestApplication.cpp}, which manages the test flow, and \texttt{robotNavigationTestImplementation.cpp}, which provides the core test logic.\\


\noindent The \texttt{config} directory houses the \texttt{robotNavigationTestConfiguration.ini} file, which defines test parameters such as goal positions and navigation constraints, allowing customization of test cases. The \texttt{data} folder contains \texttt{actionNavigationInput.dat}, \texttt{navigationServiceInput.dat}, \texttt{astarAlgorithmInput.dat}, \texttt{bfsAlgorithmInput.dat}, \texttt{boundaryTestInput.dat}, \texttt{dijkstraAlgorithmInput.dat} which stores predefined navigation goals for the tests, and \texttt{robotNavigationTestOutput.dat}, which captures the test execution logs and outputs, recording timestamps and goal completion statuses for debugging and validation.
...
\begin{enumerate}
    \item \textbf{TestPathPlanningBfs}: Validates BFS path planning algorithm via service call with test scenarios from \texttt{bfsAlgorithmInput.dat}.
    \item \textbf{TestPathPlanningDijkstra}: Validates Dijkstra path planning algorithm via service call with test scenarios from \texttt{dijkstraAlgorithmInput.dat}.
    \item \textbf{TestPathPlanningAstar}: Validates A* path planning algorithm via service call with test scenarios from \texttt{astarAlgorithmInput.dat}.
    \item \textbf{TestBoundaryConditions}: Validates rejection of out-of-bounds and unreachable goals using scenarios from \texttt{boundaryTestInput.dat}.
    \item \textbf{TestNavigationService}: Validates the navigation service interface with goals from \texttt{navigationServiceInput.dat}.
    \item \textbf{TestAlgorithmComparison}: Compares execution time across BFS, Dijkstra, and A* algorithms on the same navigation scenario.
    \item \textbf{TestActionNavigation}: Validates navigation goal execution via the action interface with scenarios from \texttt{actionNavigationInput.dat}.
    \item \textbf{TestActionFeedback}: Verifies that real-time feedback fields (distance remaining, current pose) are published during action-based navigation.
    \item \textbf{TestActionPreemption}: Verifies that an active navigation goal can be cancelled via the action cancel mechanism.
    \item \textbf{TestSetPose}: Validates pose setting via both the service and action interfaces.
\end{enumerate}

\subsection{Executing the Robot Navigation Unit Test}
To start the unit test, there are two approaches, using rosrun or using roslaunch. If you use the rosrun command, execute it as follows, and make sure both \verb|robotLocalization| and \verb|robotNavigation| nodes are running. If they are not already running use the following commands in two separate terminals.
In Terminal 1:
\begin{lstlisting}[style=withoutNumbering, language=bash]
cd ~/workspace/pepper_rob_ws
\end{lstlisting}
...
\end{lstlisting}
Upon execution, the test sequence is initiated, and results are logged in the \texttt{robotNavigationTestOutput.dat} file. If the two nodes are not running properly before the Unit test command is executed you will see the following output on the terminal, waiting for the node to run and publish proper topics and services.
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/rosrun_navigation_test.png}
    \caption{Screenshot of the unit test command running and requesting for the set\_goal service to be available.}
    \label{fig:unit_test_rosrun_terminal}
\end{figure}

\noindent If you follow the roslaunch approach, execute the following commands.\\
In Terminal 1:
\begin{lstlisting}[style=withoutNumbering, language=bash]
cd ~/workspace/pepper_rob_ws
\end{lstlisting}
...
\end{lstlisting}
Upon execution, the test sequence is initiated, and results are logged in the \texttt{robotNavigationTestOutput.dat} file. here is the sample output from the terminal.
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Images/roslaunch_navigation_test.png}
	\caption{Screenshot of the unit test command running through roslaunch.}
	\label{fig:unit_test_roslaunch_terminal}
\end{figure}
...
\begin{description}
	...
	\item [Version 1.3]~\\
	Combined service-based and action-based navigation into a single node providing both interfaces simultaneously. \\
	Added action definitions (setGoal.action, setPose.action) with goal, result, and feedback messages. \\
	Added SLAM navigation mode with automatic launch of the ROS navigation stack (map\_server, AMCL, move\_base, RViz) as a subprocess. \\
	Integrated SLAM configuration files into the robotNavigation node under the slam/ subdirectory. \\
	Added passingDistance reading from cultureKnowledgeBaseInput.dat when socialDistance mode is enabled. \\
	Added navigationMode parameter to the configuration file. \\
	Updated file organization diagram, services supported table, and added actions supported table. \\
	Added action-based interaction commands and pose setting commands. \\
	Updated unit test suite from 6 to 10 test cases, adding action navigation, feedback, preemption, and set pose tests. \\
	Birhanu Shimelis Girma. \\
	05 April 2026.
