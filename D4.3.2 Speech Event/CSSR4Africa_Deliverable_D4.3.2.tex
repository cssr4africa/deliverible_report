...
% Redefine the standard \paragraph command (already part of LaTeX, unlike
% titlesec's custom \subsubsubsection) as a numbered, own-line 4th-level
% heading, so we get the same visual result without loading titlesec (which
% corrupts \thetitle once sectioning starts -- see git history for details).
\makeatletter
...
% Correct formatting for paragraph (4th-level heading) in TOC
...
\newpage
\paragraph{Decoder/Prediction Network Architecture}
...
\newpage
\paragraph{Joint Network Architecture}
...
\paragraph{Combined Model Architecture}
