...
{\small Task No.}                               & {\small Node }                                                 &  {\small Change}       \\
\hline
{\footnotesize 2.4 }  & {\footnotesize \verb+behaviorController+}     & {\footnotesize Have two variants of the lab tour use case: (a) the robot operates autonomously, without requiring someone to introduce it (detects a visitor, ascertains mutual gaze, asks visitor if he/she would like a tour), and (b) someone introduces Pepper and initiates the tour demo (omits visitor detection).} \\ 
\hline
...
\hline
{\footnotesize 4.4, 5.6 }  & {\footnotesize All nodes}     & {\footnotesize  Save logs to file for future auditing and diagnosis of issues that may arise when the ROS nodes are running.} \\
\hline
\end{tabularx}
...
\item [Version 1.3]~\\
Updated Table \ref{table:phase1b}: added saving of logs to file for all ROS nodes, re-phrased the first item to shorten it so that the table fits within a single page.\\
Clifford Onyonka.\\
11 January 2026. 
