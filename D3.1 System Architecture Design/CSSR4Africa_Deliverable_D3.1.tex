...
\subsection{Robot Mission Interpreter}
%=================================================================================
...
{\small Key }                               & {\small Values }                                                 &  {\small Effect}       \\
\hline
{\footnotesize \verb+scenarioSpecification+}  & {\footnotesize \verb+<specificationFile>+}     & {\small Specifies the name of the robot mission specification file. The node will look for an xml file, excluding extension, with that name from the data folder.} \\
\hline
{\footnotesize \verb+verboseMode+}  & {\footnotesize \verb+true+, \verb+false+}     & {\small Specifies whether diagnostic data is to be printed to the terminal.} \\
\hline
{\footnotesize \verb+asrEnabled+}  & {\footnotesize \verb+true+, \verb+false+}     & {\small Specifies whether Automatic Speech Recognition is enabled on the platform or not.} \\
\hline
{\footnotesize \verb+audioMode+}  & {\footnotesize \verb+true+, \verb+false+}     & {\small Specifies whether vocalization of audio debugging messages should be enabled or not} \\
\hline
\end{tabularx}
\end{center}
...
\subsubsection*{Topics Subscribed}
%-----------------------------
This node  subscribes to these topics, as follows. 
...
\hline
{\footnotesize \verb+/overtAttention/data+ }  & {\footnotesize \verb+/overtAttention+}    & {\small Physical robot \& Simulator  } \\ 
\hline
\end{tabularx}
\end{center}
...
\begin{center}
\begin{tabularx}{\linewidth}{| p{6.5cm} | p{3cm} | X |}
\hline 
{\small Service }                               & {\small Message Format }                            &  {\small Effect}       \\
\hline
{\footnotesize \verb+animateBehaviour/set_activation+}  & {\footnotesize \texttt{state}}    & {\small Enable or Disable the Animate Behavior mechanism.} \\ 
\hline
{\footnotesize \verb+gestureExecution/perform_gesture+}  & {\footnotesize \texttt{gesture\_type, gesture\_id, arm, deictic\_shape, gesture\_duration, bow\_nod\_angle, location\_x, location\_y, location\_z}}    & {\small Invokes the gesture subsystem to perform a type of gesture with specified coordinates. The coordinates are only necessary if the \texttt{gesture\_type} is \texttt{deictic}.} \\ 
\hline
{\footnotesize \verb+overtAttention/set_mode+}  & {\footnotesize \texttt{state, location\_x, location\_y, location\_z}}    & {\small Sets the mode of attention for the Overt Attention node to follow.} \\ 
\hline
{\footnotesize \verb+robotLocalization/set_pose+}  & {\footnotesize \texttt{x, y, theta}}    & {\small Informs the localization subsystem of the current pose of the robot} \\ 
\hline
{\footnotesize \verb+robotNavigation/set_goal+}  & {\footnotesize \texttt{goal\_x, goal\_y, goal\_theta}}    & {\small Invokes the navigation subsystem to move to the specified coordinates.} \\ 
\hline
{\footnotesize \verb+speechEvent/set_language+}  & {\footnotesize \texttt{language}}    & {\small Sets the language for the ASR to transcribe utterances to} \\ 
\hline
{\footnotesize \verb+tabletEvent/prompt_and_get_response+}  & {\footnotesize \texttt{message}}    & {\small Sends the text to be printed as a message on the Tablet.} \\ 
\hline
{\footnotesize \verb+textToSpeech/say_text+}  & {\footnotesize \texttt{message, language}}    & {\small Sends the text to be converted to an audio signal based on the language specified and played on the robot's loudspeakers.} \\ 
\hline
\end{tabularx}
\end{center}

\subsubsection*{Action goals sent}
%---------------------------------------
\label{section:robot_mission_interpreter_goals_sent}

The node interacts with the action servers detailed in the table below. Each entry specifies the action server name, the message format required, and the expected effects or observations.

\begin{center}
\begin{tabularx}{\linewidth}{| p{6.5cm} | X | X |}
\hline 
{\small Service }                               & {\small Message Format }                            &  {\small Effect}       \\
\hline
{\footnotesize \verb+gestureExecution/perform_gesture+}  & {\footnotesize \texttt{gesture\_type, gesture\_id, arm, deictic\_shape, gesture\_duration, bow\_nod\_angle, location\_x, location\_y, location\_z}}    & {\small Invokes the gesture subsystem to perform a type of gesture with specified coordinates. The coordinates are only necessary if the \texttt{gesture\_type} is \texttt{deictic}.} \\ 
...
{\footnotesize \verb+robotNavigation/set_goal+}  & {\footnotesize \texttt{goal\_x, goal\_y, goal\_theta}}    & {\small Invokes the navigation subsystem to move to the specified coordinates.} \\ 
\hline
{\footnotesize \verb+speechEvent/recognise_speech_action+}  & {\footnotesize \texttt{language}}    & {\small Sets the language for the ASR to transcribe utterances to} \\ 
\hline
{\footnotesize \verb+textToSpeech/say_text+}  & {\footnotesize \texttt{message, language}}    & {\small Sends the text to be converted to an audio signal based on the language specified and played on the robot's loudspeakers.} \\ 
\hline
\end{tabularx}
...
\subsubsection*{Services Supported}
%---------------------------------------
 This node  provides and advertizes a server for a service {\small \verb+/gestureExecution/perform_gesture+} to initiate the performance of a required gesture.
...
\hline
{\footnotesize \verb+gesture_type+}  & {\footnotesize \texttt{iconic, symbolic}}    & {\footnotesize \texttt{String}} & {\footnotesize } \\ 
{\footnotesize }  & {\footnotesize \texttt{deictic, bow, nod}}    & {\footnotesize } & {\footnotesize } \\ 
\hline
{\footnotesize \verb+gesture_id+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Integer}} & {\footnotesize } \\ 
\hline
{\footnotesize \verb+arm+}  & {\footnotesize \texttt{LEFT, RIGHT, EITHER}}    & {\footnotesize \texttt{String}} & {\footnotesize } \\ 
\hline
{\footnotesize \verb+deictic_shape+}  & {\footnotesize \texttt{PALM\_UPWARDS, PALM\_DOWNWARDS}}    & {\footnotesize \texttt{String}} & {\footnotesize } \\ 
\hline
{\footnotesize \verb+gesture_duration+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Integer}} & {\footnotesize \texttt{milliseconds}} \\ 
\hline
{\footnotesize \verb+bow_nod_angle+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Integer}} & {\footnotesize \texttt{degrees}} \\ 
\hline
{\footnotesize \verb+location_x+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Real}} & {\footnotesize \texttt{metres}} \\ 
\hline
{\footnotesize \verb+location_y+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Real}} & {\footnotesize \texttt{metres}} \\ 
\hline
{\footnotesize \verb+location_z+}  & {\footnotesize \texttt{<number>}}    & {\footnotesize \texttt{Real}} & {\footnotesize \texttt{metres}} \\ 
\hline
\end{tabularx}
\end{center}
...
This node provides and advertises an action server at \texttt{/gestureExecution/perform\_gesture}, which is used to initiate the execution of a specified gesture. It relies on a package-specific action definition, \texttt{gesture.action}. The action definition includes the following fields:
...
\subsection*{Actions Supported}
This node provides and advertises an action server at {\small \verb+/robotNavigation/set_goal+} to request navigation to a given goal position and orientation.
The goal is sent by  the {\small \verb+behaviorController+} node. \\

The table below summarizes the action server advertized and the message format.
...
The node has one input: an audio signal that is captured by the  {\small \verb+soundDetection+} module, published on a  topic named {\small \verb+soundDetection/signal+}. When the {\small \verb+soundDetection+} module is not available or is not publishing  to the  {\small \verb+soundDetection/signal+} topic, the  {\small \verb+speechEvent+} ROS node flags this by producing a text that reads ``Error: soundDetection is down''. The node has one output: a string representing the message in the spoken audio signal.

The node can run in normal mode or verbose mode. In verbose mode, the generated text transcription is printed to the terminal while also being displayed on a graphical application that is automatically opened on the screen (this graphical application is only opened when the ROS node is running in verbose mode).

Speech transcription is triggered by a call made to the {\small \verb+speechEvent/recognise_speech_action+} ROS action. This is done via the {\small \verb+speechEvent/recognise_speech_action/goal+} ROS topic that is tied to this ROS action. Results are then published on the \\{\small \verb+speechEvent/recognise_speech_action/result+} ROS topic that is tied to this ROS action.
...
\hline
{\footnotesize \verb+model+}  & {\footnotesize \makecell[l]{\verb+conformer-transducer+, \\ \verb+parakeet+, \verb+whisper+}} &  {\small Specifies the ASR model to be used.} \\ 
\hline
{\footnotesize \verb+verboseMode+}  & {\footnotesize \verb+true+, \verb+false+}     & {\small Specifies whether diagnostic data is to be printed to the terminal.} \\
\hline
...
\hline
{\footnotesize \verb+vadThreshold+}  & {\footnotesize \verb+<number>+}     & {\small The confidence level on a scale of 0 to 1 above which an utterance is considered valid in an audio signal.} \\
\hline
...
\hline
{\footnotesize \verb+interUtteranceLen+}  & {\footnotesize \verb+<number>+}     & {\small The minimum length (in seconds) between two successive utterances, below which the two are assumed to be one utterance.} \\
\hline
{\footnotesize \verb+minUtteranceLen+}  & {\footnotesize \verb+<number>+}     & {\small The minimum length (in seconds) of an utterance; shorter utterances are ignored and therefore discarded.} \\
\hline
{\footnotesize \verb+maxUtteranceLength+}  & {\footnotesize \verb+<number>+}     & {\small The maximum length (in seconds) of an utterance, after which any extra utterances are truncated.} \\
\hline
...
This node does not publish to any ROS topics.

\subsubsection*{Services Supported}
%---------------------------------------

This node  provides and advertises a server for a service {\small \verb+/speechEvent/set_language+} to set the language to be used. This service is called by  the {\small \verb+behaviorController+} node.
...
\subsubsection*{Actions Supported}

The following are the actions that the {\small \verb+speechEvent+} node supports.
 
\begin{center}
\begin{tabularx}{\linewidth}{| l | l | X|}
\hline 
{\small Action }                                                                                & {\small  Node}    &  {\small Effect}       \\
\hline
{\footnotesize \verb+/speechEvent/recognise_speech_action+ }  & {\footnotesize \verb+behaviorController+} & {\small Triggers the speech recognition process to run and transcribe speech utterances} \\ 
\hline
\end{tabularx}
\end{center}
...
\subsection*{Actions Supported}
This node provides and advertises an action server at {\small \verb+/textToSpeech/say_text+}, which is used to initiate the conversion of a text string to an audio signal, and to play that audio on the robot's loudspeakers.

It uses a standard message type, {\small \verb+std_msgs::String+} defined in {\small \verb+std_msgs/String.msg+}, to specify the text. 
The goal is sent by  the {\small \verb+behaviorController+} node. \\


\noindent The following summarizes the action servers supported.

\begin{center}
\begin{tabularx}{\linewidth}{| l | l | X|}
\hline 
{\small Action server }                                                                                & {\small Message Value}    &  {\small Effect}       \\
\hline
{\footnotesize \verb+/textToSpeech/say_text+ }  & {\footnotesize \verb+string+ \verb+<message>+}  & {\small Convert text to an audio signal and play it on the robot's loudspeakers.} \\ 
    & {\footnotesize \verb+string+}, {\footnotesize \verb+<language>+}      &  \\ 
\hline
...
\caption{The CSSR4Africa system architecture specified at the level of ROS nodes (thin border rectangles), ROS topics (black lines), ROS services (red lines), ROS actions (blue lines), and sources \& sinks for sensor topics and actuator topics (thick border rectangles).}  
...
\item [Version 2.7]~\\
Updated the Service supported for {\small \verb+gestureExecution+} \\
Updated the services called for {\small \verb+behaviorController+} \\
Added Action supported for {\small \verb+gestureExecution+}, {\small \verb+robotNavigation+}, and {\small \verb+textToSpeech+} ROS nodes \\
Added Action goals sent for {\small \verb+behaviorController+} ROS node \\
Tsegazeab Tefferi.\\
26 June 2026.
